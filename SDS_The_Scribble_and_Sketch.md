# Software Document Specification (SDS)
...
> [!NOTE]
> ระบบเวอร์ชันปัจจุบัน (v1.0) เป็นแบบ **Client-side Only** จึงมีผู้ใช้เพียงบทบาทเดียว หากมีการพัฒนาต่อในอนาคต อาจเพิ่มบทบาท Admin สำหรับจัดการข้อมูลส่วนกลางได้
...
graph TB
    subgraph Client["🖥️ Client-Side (Browser)"]
        direction TB
        UI["📄 index.html<br/>โครงสร้าง UI"]
        CSS["🎨 style.css<br/>Design System + Theme"]
        JS["⚡ app.js<br/>Application Logic (IIFE)"]
        LS["💾 LocalStorage<br/>ข้อมูล Todo + การตั้งค่า"]
    ...
    subgraph External["☁️ External CDN"]
        GF["Google Fonts<br/>Bricolage Grotesque<br/>Plus Jakarta Sans"]
        MS["Material Symbols<br/>Outlined Icons"]
    end
    
    subgraph Deploy["🚀 Hosting"]
        VCL["Vercel<br/>Static Site Hosting"]
    end
...
แผนภาพ Use Case ที่แสดงฟังก์ชันหลักของระบบทั้งหมดแยกตาม Subsystem
...
flowchart LR
    ...
    subgraph TheScribbleSketch["ระบบ The Scribble & Sketch"]
        direction TB

        %% Subsystem: Todo Management
        subgraph Todo_CRUD ["ระบบจัดการรายการ Todo"]
            direction TB
            UC1(["เพิ่มรายการใหม่"])
        ...
        subgraph Status_Mgmt ["ระบบจัดการสถานะ"]
            direction TB
            UC6(["เปลี่ยนสถานะผ่าน Checkbox"])
            UC7(["เปลี่ยนสถานะผ่าน Dropdown"])
            UC8(["ลากย้ายการ์ด Drag & Drop"])
        end

        %% Subsystem: View & Filter
        subgraph View_Filter ["ระบบมุมมองและตัวกรอง"]
            direction TB
        ...
        %% Subsystem: Settings
        subgraph Settings ["ระบบตั้งค่า"]
            direction TB
            UC11(["สลับ Dark/Light Mode"])
        end

        %% Cross-boundary dependencies
        UC1 -.->|"สร้าง Todo → สถานะเริ่มต้น todo"| UC6
        UC8 -.->|"ลากการ์ด → อัปเดตสถานะ"| UC7
    end
...
> [!NOTE]
> ระบบเวอร์ชันปัจจุบันใช้ **LocalStorage** เป็นที่จัดเก็บข้อมูล ไม่ได้ใช้ Relational Database แต่โครงสร้างข้อมูลสามารถอธิบายในรูปแบบ ERD ได้ดังนี้
...
flowchart TD
    ...
    D -->|ไม่มี| F["Migrate: status = done ? 'done' : 'todo'"]
    F --> G{"มี field done?"}
    G -->|ไม่มี| H["เพิ่ม done = status === 'done'"]
    G -->|มี| I["ใช้ค่าเดิม"]
...
flowchart TD
    ...
    B --> C{"ข้อความว่างเปล่า<br/>หลัง trim?"}
    C -->|ใช่| D["ไม่ทำอะไร — Return"]
    C -->|ไม่ใช่| E["สร้าง Object: id, text, status='todo',<br/>done=false, createdAt=Date.now()"]
    E --> F["เพิ่ม Object ไว้ด้านบนสุดของ Array<br/>ด้วย unshift()"]
...
### 7.2 Activity Diagram: เปลี่ยนสถานะผ่าน Drag & Drop (Board View)

```mermaid
flowchart TD
    Start(["เริ่มต้น"]) --> A["ผู้ใช้กดค้างที่การ์ด Todo"]
    A --> B["เริ่ม Drag:<br/>บันทึก todo.id + เพิ่ม class dragging"]
    B --> C["ลากการ์ดเข้าพื้นที่ Column เป้าหมาย"]
    C --> D["Column เป้าหมายแสดง<br/>Visual Feedback (เส้นประ + พื้นหลังเปลี่ยนสี)"]
    D --> E["ผู้ใช้ปล่อยการ์ด (Drop)"]
    E --> F{"ตรวจสอบ: มี todo.id<br/>และ targetStatus?"}
    F -->|ไม่มี| G["ไม่ทำอะไร"]
    F -->|มี| H["อัปเดต todo.status<br/>= targetStatus ของ Column"]
    H --> I["อัปเดต todo.done<br/>= status === 'done'"]
...
flowchart TD
    ...
    A --> B{"ตรวจสอบ:<br/>ธีมปัจจุบันเป็น Dark?"}
    B -->|ใช่ → เปลี่ยนเป็น Light| C["ลบ class 'dark'<br/>จาก html + body"]
    C --> D["เปลี่ยนไอคอนเป็น 🌙 dark_mode"]
    D --> E["บันทึก theme='light'<br/>ลง LocalStorage"]
    B -->|ไม่ใช่ → เปลี่ยนเป็น Dark| F["เพิ่ม class 'dark'<br/>ให้ html + body"]
    F --> G["เปลี่ยนไอคอนเป็น ☀️ light_mode"]
    G --> H["บันทึก theme='dark'<br/>ลง LocalStorage"]
    E --> End(["จบ"])
    H --> End
```
...
flowchart TD
    ...
    A --> B{"มีรายการ<br/>สถานะ done?"}
    B -->|ไม่มี| C["ไม่ทำอะไร — Return"]
    B -->|มี| D["วนลูปรายการ done ทั้งหมด"]
    D --> E["เพิ่ม class removing + Animation Delay<br/>ให้แต่ละรายการ"]
    E --> F["รอ Animation เสร็จ<br/>(300ms + items × 50ms)"]
    F --> G["Filter: ลบ todo ที่ status === done<br/>ออกจาก Array"]
...
flowchart TD
    subgraph SinglePage ["📄 Single Page Application (index.html)"]
        ...
        Header["🏷️ Header<br/>Logo + Theme Toggle + Menu"]
        Hero["🎯 Hero Section<br/>Badge + Title + Subtitle"]
        Form["📝 Add Form<br/>Input + Submit Button"]
        Toolbar["🔧 Toolbar<br/>Filter Tabs + View Switcher"]
        
        subgraph Views["📊 Content Views (สลับกัน)"]
            ListView["📋 List View<br/>Stats Bar + Todo Cards (แนวตั้ง)"]
            BoardView["📌 Board View / Kanban<br/>3 Columns: To Do | Doing | Done"]
        end
        
        EmptyState["🎨 Empty State<br/>ไอคอน + ข้อความเมื่อไม่มีรายการ"]
        Footer["📎 Footer<br/>Brand + Copyright"]
        
        Header --> Hero --> Form --> Toolbar --> Views --> EmptyState --> Footer
    end
```

### 9.2 Wireframe: โครงสร้างหน้าจอหลัก

```
┌──────────────────────────────────────────┐
│  🏷️ The Scribble & Sketch    [🌙] [✏️]  │  ← Header
├──────────────────────────────────────────┤
│          ╭ Creative Todo ╮               │
│       My Creative Archive                │  ← Hero Section
│   จดทุกไอเดีย วางแผนทุกโปรเจกต์...       │
├──────────────────────────────────────────┤
│  ┌──────────────────────────┐  ┌──────┐ │
│  │ เขียนสิ่งที่ต้องทำ...     │  │ เพิ่ม │ │  ← Add Form
│  └──────────────────────────┘  └──────┘ │
├──────────────────────────────────────────┤
│  [ทั้งหมด 5] [กำลังทำ 2] [เสร็จ 3]      │
│                          [📋 List] [📌]  │  ← Toolbar
├──────────────────────────────────────────┤
│  5 รายการ · 3 เสร็จแล้ว    [ลบที่เสร็จ]  │  ← Stats Bar
├──────────────────────────────────────────┤
│  ┌────────────────────────────────────┐  │
│  │ ☐ ซื้อสมุดโน้ตใหม่                  │  │
│  │   17 ก.ค. 23:00    [To Do ▾]  ✏️ ✕ │  │  ← Todo Item
│  └────────────────────────────────────┘  │
│  ┌────────────────────────────────────┐  │
│  │ ┃ ☐ วาดรูปส่งลูกค้า                │  │
│  │   17 ก.ค. 22:30    [Doing ▾]  ✏️ ✕ │  │  ← Doing Item
│  └────────────────────────────────────┘  │
│  ┌────────────────────────────────────┐  │
│  │ ☑ ̶อ̶่า̶น̶ห̶น̶ัง̶ส̶ือ̶                     │  │
│  │   17 ก.ค. 20:00    [Done ▾]  ✏️ ✕  │  │  ← Done Item
│  └────────────────────────────────────┘  │
├──────────────────────────────────────────┤
│        The Scribble & Sketch             │
│   Crafted for the curious mind. © 2024  │  ← Footer
└──────────────────────────────────────────┘
```

### 9.3 Wireframe: Board View (Kanban)

```
┌──────────────────────────────────────────────────────┐
│  สิ่งที่ต้องทำ [2]  │  กำลังทำ [1]    │  เสร็จแล้ว [2]  │
│  ─────────────────  │  ─────────────  │  ──────────────  │
│  ┌───────────────┐  │  ┌───────────┐  │  ┌────────────┐  │
│  │ ซื้อสมุดโน้ต   │  │  │ วาดรูป    │  │  │ ̶อ̶่า̶น̶ห̶น̶ัง̶ส̶ือ̶  │  │
│  │ 17 ก.ค.       │  │  │ 17 ก.ค.   │  │  │ 17 ก.ค.    │  │
│  └───────────────┘  │  └───────────┘  │  └────────────┘  │
│  ┌───────────────┐  │                 │  ┌────────────┐  │
│  │ ซื้อดินสอสี    │  │                 │  │ ̶ส̶่ง̶ง̶า̶น̶      │  │
│  │ 16 ก.ค.       │  │                 │  │ 16 ก.ค.    │  │
│  └───────────────┘  │                 │  └────────────┘  │
│                     │                 │                  │
...
### 9.4 Design System: Color Palette
...
|------|------------|------|--------|----------|
| Headline XL | Bricolage Grotesque | 48px | 800 | — (Reserve) |
| Headline LG | Bricolage Grotesque | 32px | 700 | — (Reserve) |
...
> [!IMPORTANT]
> ระบบเวอร์ชันปัจจุบัน (v1.0) เป็นแบบ **Client-side Only** จึง **ไม่มี Backend API** ข้อกำหนดในส่วนนี้เป็นการระบุ **Internal API** (LocalStorage Interface) ที่ใช้ภายในระบบ และ **External Resources** ที่ระบบเรียกใช้
...
| **Return** | `string \| null` — JSON string ของ Todo Array |
...
| รายการ | รายละเอียด |
|-------|-----------|
| **Read** | `localStorage.getItem('creative-archive-theme')` → `"light" \| "dark" \| null` |
...
| รายการ | รายละเอียด |
|-------|-----------|
| **Read** | `localStorage.getItem('creative-archive-view')` → `"list" \| "board" \| null` |
...
|----------|--------|-----|---------|
| Google Fonts | `GET` (CSS) | `https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:wght@600;700;800&family=Plus+Jakarta+Sans:wght@400;500;600;700` | โหลด Web Fonts |
| Material Symbols | `GET` (CSS) | `https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:wght,FILL@100..700,0..1` | โหลด Icon Font |
| Paper Texture | `GET` (Image) | `https://www.transparenttextures.com/patterns/natural-paper.png` | พื้นหลังลายกระดาษ |
...
| **CSS Custom Properties** | (หรือ CSS Variables) ตัวแปรใน CSS ที่ใช้ `--ชื่อ` สำหรับสร้างระบบ Design Token ที่เปลี่ยนค่าได้ตามบริบท (เช่น Light/Dark) |
